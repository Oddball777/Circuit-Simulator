...
class Circuit:
    ...
    def make_truth_table(self) -> None:
    ...
    def get_sources_values(self) -> np.ndarray:
        return np.array(list(product(range(2), repeat=len(self.sources))))

    def get_sources_series(self, source_values) -> list[pd.Series]:
        series_of_sources = [
    ...
    def get_indicators_series(self, source_values) -> list[pd.Series]:
    ...
    def get_indicators_values(self, source_values) -> np.ndarray:
        indicator_values = []
